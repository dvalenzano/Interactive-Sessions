...
z = open('/Volumes/group_dv/personal/DValenzano/Mar2014/fam_1.1.Onemap.txt', 'w')
z.write(fin)
z.close()
...
for f in files:
    ...
    if len(fams) < 11:
        ...
        len(lz5.split('\n')[0].split(','))
        fin = str(len(lz5.split('\n')[0].split(',')))+' '+str(len(lz5.split('\n')[:-1]))+'\n'+lz5
        o = '/Volumes/group_dv/personal/DValenzano/Mar2014/OneMap/'+ ','.join(f.split('-')[1].split('.')[:-1]).replace(',','.')+ '_OneMap.txt'
        z = open(o, 'w')
        z.write(fin)
        z.close()
