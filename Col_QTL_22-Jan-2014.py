...
go_7T = zip(*[ i.split(',')  for i in go_7 ]) #Now I am transposing go_7

# <codecell>

gO = ','.join([ ','.join(list(i))+'\n'  for i in go_7T]).replace('\n,','\n') # I am turning go_7T into a big string

# <codecell>

goS = gO.split('\n') # I am splitting g0 into a list

# <codecell>

gos_N = [goS[1]]+[goS[6]]+goS[10:] # I am only keeping the header, the color row (it's a row b/c it's been transposed) and all the genotypes
...
lsZ = ','.join(lS).replace('\n,','\n')
w = open('/Volumes/group_dv/personal/DValenzano/Jan2014/Stanford_paper/Go_fam7_Col_qtl01.tsv', 'w')
