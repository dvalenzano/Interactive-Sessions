...
class sortColumn2(object):
    ...
    def __init__(self, inp, pval, col):
        ...
        self.t2 = [','.join([i.split(',')[0]]+i.split(',')[self.col].split('_')[:2]+[self.kd[i.split(',')[0]]]) for i in self.t[1:-1]]
...
aalist = ['aa1m','aae1m','aa1f','aae1f','aa3m','aae3m']


# In[6]:

class sortColumn3(object):
    ...
    def __init__(self, inp, pval, col):
        ...
        self.t = self.tosave.split('\n')
        self.t2 = [','.join([i.split(',')[0]]+i.split(',')[self.col].split('_')[:2]+[self.kd[i.split(',')[0]]]) for i in self.t[1:-1]]
        self.t3 = [j.split(',')[0] for j in [i for i in self.t[1:-1] if i.split(',')[8]!= ''] if sum(map(float, j.split(',')[8:]))<4] 
        self.t4 = [k.split(',')[0] for k in  [j for j in [i for i in self.t[1:-1] if i.split(',')[8]!= ''] if sum(map(float, j.split(',')[8:]))<4] if k.split(',')[7]=='-' ]
        self.ls = []
...
for i in aalist:
    ls = '/Volumes/group_dv/personal/DValenzano/Jul2014/aasort_%s.csv' % i
    p = sortColumn3(aa,aapval, i)
    z = open(ls, 'w')
    z.write(p.tosave)
    z.close() #this way I can save several files at once
    lz = '/Volumes/group_dv/personal/DValenzano/Jul2014/rinput_%s.csv' % i
